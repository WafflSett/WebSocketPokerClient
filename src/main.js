...
const connect = () => {
  ...
  ws.onopen = () => {
    ...
    document.querySelector("#navForm").classList.remove("d-none")
  ...
  ws.onmessage = event => {
    ...
    if (msg.type == "disc") {
      document.querySelector("#p" + msg.position).classList.add("d-none")
      return
    }
    if (msg.type == "start") {
      login.disabled = true;
      ...
      document.querySelector("#betAmount" + msg.sBlind).innerHTML = (
        msg.bet / 2
      ).toString()
      document.querySelector(
        "#betAmount" + msg.bBlind
      ).innerHTML = msg.bet.toString()
      return
    }
    if (msg.type == "blind") {
    ...
    if (msg.type == "upnext") {
      ...
      document.querySelectorAll(".playerPicture").forEach(pX => {
        let idPos = pX.parentElement.id
        if (Number(idPos[1]) == msg.position) {
          if (Number(idPos[1]) == position) {
            pX.style.filter =
              "sepia(100%) saturate(300%) brightness(80%) hue-rotate(180deg)"
          } else {
            pX.style.filter = "brightness(1)"
          }
        } else {
          if (Number(idPos[1]) == position) {
            pX.style.filter =
              "sepia(100%) saturate(300%) brightness(45%) hue-rotate(180deg)"
          } else {
            pX.style.filter = "brightness(.45)"
          }
        }
      })
      displayBets(msg.userList)
      if (msg.position == position) {
        ...
        startTimer()
        document.querySelector("#action-btnsMainDiv").classList.remove("d-none")
        if (msg.runningBet > 0) {
          ...
          raiseBtn.addEventListener("click", () => {
            if (
              myBalance > 0 &&
              Number(betAmount.value) > msg.runningBet &&
              myBalance - Number(betAmount.value) >= 0
            ) {
          ...
          document.querySelector("#betField").append(raiseBtn)
          ...
          document.querySelector("#betField").append(betBtn)
        ...
        allinBTN.addEventListener("click", () => {
          if (myBalance > 0) {
            ...
            myBalance = 0
            timerOn = false
            document.querySelector("#balance").innerHTML = "0 Ft"
            clearBTNs()
          }
        })
        if (myBalance <= 0) {
    ...
    if (msg.type == "hand") {
      ...
      const firstCard = msg.hand[0]
      const secondCard = msg.hand[1]
      document.querySelector(
        "#firstCard"
      ).src = `src/images/${firstCard[0]}/${firstCard}.png`
      document.querySelector(
        "#secondCard"
      ).src = `src/images/${secondCard[0]}/${secondCard}.png`
      return
    }
    if (msg.type == "roundend") {
      ...
      myBet = 0
      let communityCards = document.querySelector("#communityCards")
...
const startTimer = async () => {
  timerOn = false
  let timer = document.querySelector("#timerBar")
...
const showOnlineUsers = userList => {
  document.querySelectorAll(".profile").forEach(p => {
    p.classList.add("d-none")
  })
  userList?.forEach(u => {
    let pX = document.querySelector("#p" + u.position)
    pX.classList.remove("d-none")
    pX.children[1].innerHTML = u.userName
...
const displayBets = userlist => {
  userlist.forEach(user => {
    document.querySelector(
      `#betAmount${user.position}`
    ).innerHTML = user.bet.toString()
  })
}
...
const sdProfileCreater = msg => {
  let pot = msg.pot
  let userList = msg.userList
  document.querySelector("#showdownWindow").classList.remove("d-none")
  let sdProfiles = document.querySelector("#sdProfiles")
...
let readyBtn = document.querySelector("#readyUp")
let unreadyBtn = document.querySelector("#unready")
...
const reset = () => {
  ...
  document.querySelector("#firstCard").src = ""
  document.querySelector("#secondCard").src = ""
  document.querySelector(
    ".table-container"
  ).innerHTML = `<img src="src/images/table.png" class="table-image" alt="table"><p id="pot"></p><div id="communityCards"></div>`
  potP = document.querySelector("#pot")
  createProfiles()
}
